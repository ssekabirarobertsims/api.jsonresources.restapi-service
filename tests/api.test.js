...
describe("/", () => {
    ...
    test("test for root route for the api", async () => {
        ...
        expect(response.headers["content-type"]).toStrictEqual(expect.stringContaining("html"));
...
describe("/resources/texts", () => {
    ...
    test("test deleting texts resources", async () => {
        const response = await request(application).delete("/resources/texts/242c59c1-dba4-4e1c-a2eb-056b12e8d32e");
        expect(response.statusCode).toEqual(200);
        expect(response.headers["content-type"]).toStrictEqual(expect.stringContaining("json"));
    });
...
describe("/resources/users", () => {
    ...
    test("test deleting users resources", async () => {
        const response = await request(application).delete("/resources/users/030f3d31-5209-482e-a391-ae669a1b7f04");
        expect(response.statusCode).toEqual(200);
        expect(response.headers["content-type"]).toStrictEqual(expect.stringContaining("json"));
    });
...
describe("/resources/photos", () => {
    ...
    test("test deleting photos resources", async () => {
        const response = await request(application).delete("/resources/photos/fb633db6-e82e-4ccc-897b-ec3baw37899be");
        expect(response.statusCode).toEqual(200);
        expect(response.headers["content-type"]).toStrictEqual(expect.stringContaining("json"));
    });
...
describe("/resources/posts", () => {
    ...
    test("test deleting posts resources", async () => {
        const response = await request(application).delete("/resources/posts/fb633db6-e8e-4ccc-897b-ec3ba37899be");
        expect(response.statusCode).toEqual(200);
        expect(response.headers["content-type"]).toStrictEqual(expect.stringContaining("json"));
    });
